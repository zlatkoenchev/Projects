...
char HF_Out_Data[CDC_DATA_IN_EP_SIZE];

unsigned char  NextUSBOut;
unsigned char    NextUSBOut;
unsigned char    HF_TxLen;  // Number of characters in the buffer
unsigned char    RS232cp;       // current position within the buffer
unsigned char HF_Out_Data_Rdy = 0;
...
static void InitializeSystem(void) {
    ...
    #if defined(PIC18F87J50_PIM) || defined(PIC18F46J50_PIM) || defined(PIC18F25J50_PIM)
    ...
    #if defined(PIC18F46J50_PIM) || defined(PIC18F25J50_PIM)
	//Configure all I/O pins to use digital input buffers.  The PIC18F87J50 Family devices
	//use the ANCONx registers to control this, which is different from other devices which
	//use the ADCON1 register for this purpose.
    ANCON0 = 0xFF;                  // Default all pins to digital
